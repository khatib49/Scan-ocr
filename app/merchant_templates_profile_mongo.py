...
router = APIRouter(
    prefix="/merchant-templates-profiles-mongo",
    tags=["Merchant Templates Profiles (Mongo)"],
)

_profiles_lock = asyncio.Lock()
COLL = lambda: DB[
    "MerchantsTemplatesProfile"
]  # Mongo collection for merchant templates profiles
...
async def get_all_profiles(
    q: Optional[str] = Query(
        None,
        description="Case-insensitive search in MerchantName_Keyword or MerchantAddress_Keyword",
    ),
...
async def find_similar_profile(
    merchant_guess: str, address_guess: str = None
) -> Dict[str, Any]:
    """
    ...
    def get_candidate_addresses(doc: Dict[str, Any]) -> List[str]:
        ...
        # Primary address
        if (
            isinstance(prof.get("MerchantAddress"), str)
            and prof["MerchantAddress"].strip()
        ):
            addresses.append(prof["MerchantAddress"].strip())
        if (
            isinstance(doc.get("MerchantAddress"), str)
            and doc["MerchantAddress"].strip()
        ):
    ...
    if receipt_has_address:
        ...
        profile_has_address = bool(
            (prof.get("MerchantAddress") and str(prof.get("MerchantAddress")).strip())
            or (
                best_doc.get("MerchantAddress")
                and str(best_doc.get("MerchantAddress")).strip()
            )
            or (
                prof.get("MerchantAddress_Keyword")
                and len(prof.get("MerchantAddress_Keyword", [])) > 0
            )
            or (
                best_doc.get("MerchantAddress_Keyword")
                and len(best_doc.get("MerchantAddress_Keyword", [])) > 0
            )
        )
        ...
        if not profile_has_address or addr_score == 0.0:
            ...
            matched = False
            rejection_reason = (
                "Receipt has address but profile has no address to compare"
            )
        elif (
            name_score >= NAME_MIN_THRESHOLD_STRICT
            and addr_score >= ADDRESS_MIN_THRESHOLD
        ):
        ...
        if profile_has_address:
            ...
            matched = False
            rejection_reason = (
                "Receipt has no address but profile requires address verification"
            )
